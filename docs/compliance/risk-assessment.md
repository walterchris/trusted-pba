...
| R-014 | `pba-override` Secure Boot override boots an out-of-`db` image | A2/A5/A6 | I/A | Low | High | **Low** | Gated — `-tags trustbroker` + explicit `pba-override` policy opt-in, off by default (not in release builds); fail-closed (enforcing-SB precondition + verify before arm, one-shot disarm, restore). PCR-7 divergence characterized. **Accepted for the non-Windows/custom-loader path** (ADR-0013, PR #88, 2026-07-05; review `evidence/security-review-records/2026-07-05-pba-override-trust-broker-82.md`). **Windows extension (ADR-0014, Proposed):** for our-keys-only platforms (firmware `db` = key A only, Microsoft Windows CAs in the PBA's own trust store) the override now also brokers Windows Boot Manager, so it is **no longer blanket "scoped away from Windows"** — the fail-closed authorization properties (verify-before-arm, one-shot, restore) are unchanged and evidenced by the negative demo (broker without the Windows CAs → `bootmgfw` rejected, override never armed). The **new residual is operational, not authorization** (PCR-7/BitLocker recovery-key prompt on a PBA/override change), mitigated by ADR-0014's mandatory provisioning obligation (seal BitLocker with the PBA already measured; PBA updates = reseal events) and bounded by an OPEN pre-condition (verify the PCR-7 seal profile vs a primary Microsoft source + validate on real HW before any production Windows use). **Measured-boot extensions (ADR-0015/0016, Proposed):** the override now also *measures* itself — ADR-0015 records its own authorization into PCR 7 (`EV_EFI_VARIABLE_AUTHORITY`, PBA-owned namespace + the trust-store CA that validated the image) and ADR-0016 measures the chained image (`bootmgfw`/UKI/…) into policy-configured PCR(s) (default `[4]`) via `EFI_TCG2_PE_COFF_IMAGE`, restoring the boot-application measurement `LoadImageBuffer` skips. This makes the override **attestable instead of silent** and lets BitLocker bind the full default `0,2,4,11` profile under the override (emulation-validated: reseal + 5/5 reboots to logon; fresh TPM → recovery). **Authorization is UNCHANGED** — it still rests on `internal/imageverify` + the `internal/tbstub` arm/verify gate, and the measurement is advisory-to-attestation, running post-verify/post-arm (independent security review this session: VERDICT **APPROVE**, security-neutral-to-positive, all findings LOW). The **fail-closed control on the measurement path is the new `require_tpm` policy flag** (default `false`): `true` → a missing TPM or a failed extend refuses to boot (do not boot an image we cannot measure); `false` → best-effort (log loudly, boot the verified image *unattested* — never a silent fallback). The residual for adopting/changing these measurements (or for any PBA change) stays **operational, not authorization** — a one-time BitLocker reseal event (the managed case), not a Secure Boot bypass; **open pre-condition:** the PCR-4 `PE_COFF` value is firmware-dependent and unproven off-hardware (ADR-0016 F-1 — real-HW validation DEFERRED by decision this session). ADR-0015/0016 are **Proposed**, gated behind `-tags trustbroker`, and require the §5.3 Security/Release Owner human gate before Accepted; release builds stay override-free. ADR-0014 is likewise **Proposed** — pending the §5.3 human gate + independent security review; release defaults stay override-free. threat-model TB2/R-014; #82 |
...
- **Mitigations:** `pba` mode computes the Authenticode hash, checks `dbx`-by-hash
  ...
  `ExtKeyUsageCodeSigning`, and applies `dbx`-by-cert across the signer's **whole
  PKCS#7 bundle** — every cert the signer stapled, not only the leaf + the chain(s)
  `x509.Verify` returns (#91) — plus a per-chain revocation loop that additionally
  covers a revoked `db` root (not part of the bundle); any failure → fail closed
  before `LoadImage`. `firmware` mode defers to firmware Secure Boot.
- **Residual:** Low (parser-bug risk in `go-uefi`, bounded by pinning + fuzz).
- **Tests:** `TestVerifyFailsClosed/*` — incl. **`revoked bundled cert off the
  winning chain (#91)`** (a leaf that chains directly to the `db` root while the
  signer also staples a dbx-revoked intermediate `x509.Verify` routes around →
  rejected `ErrRevokedCert`; mutation-proven — reverting the full-bundle loop makes
  it the only failing subtest) — and `TestVerifyAccepts`; `pba-matrix`
...
- **Derive stage (ADR-0011 §3, #104, A4a; #112):** orthogonal to the source, an optional
  ...
  default (hash-provisioned) drive. The iteration count + key length are
  **policy-configurable** via an optional `derive_params` block (`iterations: <int>|"auto"`,
  `key_len`; #112) — **absent means the defaults 500000/32, byte-for-byte the pre-#112
  behavior**; validation is fail-closed (stray `derive_params` on non-sedutil derives,
  explicit iterations outside 1..100_000_000, key_len outside 1..64). `auto` tries a fixed
  best-first candidate list `[500000, 75000]`, advancing only on `NOT_AUTHORIZED` and
  stopping closed on lockout / other / exhaustion — **try-limit-safe** (best-first burns no
  extra Admin1 try in the common case; the seed and every derived key are zeroized on all
  paths, F-2). `unlockSED` derives after drive selection (the salt
...
| 2026-08-16 | **R-001 — dbx-by-cert revocation now covers the whole signing bundle (#91, branch `fix/imageverify-dbx-full-bundle-91`).** `internal/imageverify.Verify` previously checked dbx-by-cert only on the leaf plus the certs on the chain(s) `x509.Verify` returns; a dbx-revoked **intermediate the signer stapled into the PKCS#7 bundle** that `x509.Verify` routes around (cross-signing / an alternate path to `db`) was never consulted — a **revocation bypass** (fail-open on the pba-mode image-acceptance path, exactly the "never ignore revocation" invariant). The fix checks dbx-by-cert against **every** cert in the signer's bundle (`slices.ContainsFunc(certs, v.revoked)`); the per-chain loop is retained (it uniquely covers a revoked `db` root, not in the bundle). Regression subtest **`revoked bundled cert off the winning chain (#91)`** is mutation-proven (reverting the fix makes it the only failing subtest — returns nil = the image would boot). This **tightens** acceptance (never weakens) and over-rejection is fail-closed-safe (a legitimate image does not bundle revoked material; Windows uses firmware mode, imageverify is pba-mode custom/recovery only); R-009 fuzz path unaffected. Touches no Secure Boot / chainloader / key / crypto behavior and does not change the trust decision beyond closing the bypass ⇒ **no ADR** (baseline §23). go-reviewer + security-review-agent both **APPROVE** (security review mutation-proved the bypass in an isolated worktree; no new fail-open). **No rating change** — residual stays **Low** (strengthened mitigation on an already-Mitigated risk). Review record: `evidence/security-review-records/2026-08-16-imageverify-dbx-full-bundle-91.md`. Supersedes the #91 open-gap note in the 2026-08-16 full-codebase audit (finding C-1 / dbx-by-cert winning-chain scope). |
| 2026-08-27 | **R-014 — `pba-override` extended to the Windows path (ADR-0014, Proposed; branch `fix/windows-handoff-demos`).** The `demo:windows` demonstrator was switched from ADR-0013's firmware-`db` Windows model (validation `pba`, Microsoft CAs enrolled in firmware `db`) to the **`pba-override`** model: firmware `db` = **our key A only**, and the PBA trust-brokers Windows Boot Manager against the Microsoft Windows CAs in the PBA's **own** embedded trust store (trust set `windows-only`), admitting it via the Security2 override — one symmetric trust model for both the Linux UKI and `bootmgfw`. ADR-0014 **reverses ADR-0013 decision #3** ("not for the Windows path") for **our-keys-only** platforms, so the R-014 mitigation is **no longer blanket "scoped away from Windows/BitLocker"**. The **fail-closed authorization properties are unchanged** (enforcing-SB precondition, verify-before-arm, one-shot pointer+size authorization, restore) and evidenced in-session by the positive demo (override boots to Windows 11 Setup — evaluation media, no BitLocker enrolled) and the **negative demo** (broker trust store **without** the Windows CAs → `bootmgfw` rejected `signer does not chain to a trusted db certificate`, `override armed` never printed → fail closed). The **new residual is operational, not authorization** — a PCR-7/BitLocker recovery-key prompt whenever the PBA/override changes — **mitigated** by ADR-0014's **mandatory provisioning obligation** (enable/seal BitLocker with the PBA-override already measured; treat PBA/override updates as PCR-7-affecting reseal events; a deployment that cannot guarantee this must use the firmware-`db` `pba`/`firmware` path instead) and bounded by an **OPEN pre-condition** (the precise BitLocker PCR-7 seal profile must be verified against a **primary Microsoft source** and the auto-unlock-after-reseal claim validated on **real hardware** before any **production** Windows deployment). **No rating change** — residual stays **Low**: the authorization posture (booting an out-of-`db` image) is unchanged and still gated + fail-closed; the new item is an availability/recovery operational residual bounded by the provisioning obligation, not a Secure Boot bypass. **Status: Proposed / pending human gate + security review — NOT Accepted.** ADR-0014's §5.3/§23 Security/Release Owner human gate and an independent security review (running separately) are merge pre-conditions (as for ADR-0013); **shipped/release defaults are unchanged** (release builds stay override-free; `release-policy` gate untouched). CRA matrix: ER-2 gap note updated (gated override capability + Proposed ADR-0014 Windows extension; shipped ER-2 posture unchanged). Threat-model TB2 exception + R-014 row + PCR-7 note refined (breaks unless sealed-with-PBA), 2026-08-27 change-log entry. |
| 2026-08-29 | **R-014 — `pba-override` becomes measured/attestable (ADR-0015 + ADR-0016, both Proposed; branch `fix/windows-handoff-demos`).** Two security-critical measured-boot changes extend the `-tags trustbroker` override (absent from release builds): **ADR-0015** measures the override's own authorization into **PCR 7** (`EV_EFI_VARIABLE_AUTHORITY`, PBA-owned namespace + the trust-store CA that validated the image) before `StartImage`, so the override is attestable instead of silent; **ADR-0016** measures the chained image (`bootmgfw`, UKI, …) into policy-configured PCR(s) — default `[4]`, explicit empty list disables — via `EFI_TCG2_PE_COFF_IMAGE`, restoring the boot-application measurement `LoadImageBuffer` skips, which lets BitLocker auto-unlock the full default **PCR 0,2,4,11** profile under the override (emulation-validated: reseal + 5/5 reboots to Windows logon; fresh TPM → recovery, genuinely gated). New policy config: **`require_tpm`** (bool, default `false`) — the fail-closed control on the measurement path: `true` → a missing TPM or a failed `HashLogExtendEvent` refuses to boot (do not boot an image we cannot measure); `false` → best-effort (log loudly, boot the verified image *unattested*, never a silent fallback); plus per-entry `measure_image_pcrs` (default `[4]`, range-checked 0–23, `pba-override` entries only). **Authorization is UNCHANGED** — it rests on `internal/imageverify` + the `internal/tbstub` arm/verify gate, not on the measurement (measurement runs post-verify, post-arm; advisory-to-attestation). New pure-Go `internal/tcgmeasure` (host-tested byte layout); the go-boot fork bumped **tpba.7 → tpba.8** — additive new file `uefi/tcg2.go` (`GetTCG2` + `HashLogExtendEvent`), no change to existing files/asm (R-006 supply-chain: additive-only, individually reviewable). **Independent security review this session: VERDICT APPROVE** (security-neutral-to-positive; all findings LOW): F-1 the PCR-4 `PE_COFF` value is firmware-dependent and unproven off-hardware (ADR-0016 open pre-condition — real-HW validation **DEFERRED** by decision this session); F-2 cosmetic `ImageLocationInMemory` (log-only); F-3 the default measure-into-PCR-4 now also applies to the ADR-0013 non-Windows override path (human-gate confirm); F-4 = this compliance update. **No rating change** — R-014 residual stays **Low**: the authorization posture (booting an out-of-`db` image) is unchanged and still gated + fail-closed; the residual for adopting/changing the measurements (or any PBA change) is an **operational, one-time BitLocker reseal** event (the managed case), not a Secure Boot bypass. **Status: both Proposed / pending the §5.3 Security/Release Owner human gate — NOT Accepted;** release builds stay override-free. Evidence: `evidence/security-review-records/2026-08-29-override-measured-boot-adr-0015-0016.md`. Mirrors threat-model 2026-08-29 (TB2/measured-boot + R-014). |
