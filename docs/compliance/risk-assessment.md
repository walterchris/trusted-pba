...
- **Derive stage (ADR-0011 §3, #104, A4a):** orthogonal to the source, an optional
  ...
  `Serial()` error, and unknown derive — never sends a credential; today's
  Storage-Security carrier is not a `Serialer`, so `sedutil-pbkdf2` fails closed until
  the NVMe-passthru carrier (A4b). The consumed seed and the new derived key are
