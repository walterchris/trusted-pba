...
- **TB2 PBA → ESP / second-stage image.** The PBA reads the target image from the
  ...
  **OPEN pre-condition**: the precise BitLocker PCR-7 seal profile must be verified
  ...
  security review are pending, and release builds stay override-free.
  **Measured-boot extensions (ADR-0015/0016, Proposed).** The override no longer only
  *authorizes* an out-of-`db` image, it now also **measures** what it did, closing the
  measured-boot gap: **ADR-0015** extends an `EV_EFI_VARIABLE_AUTHORITY` event into **PCR 7**
  (PBA-owned namespace — variable `PbaOverride`, fixed PBA `VendorGuid`/`SignatureOwner` — plus
  the embedded-trust-store `db` CA the image's signer chained to) before `StartImage`, so a
  verifier can tell that the PBA, not firmware `db`, vouched for the OS loader (an honest,
  PBA-namespaced authority — spoofing a firmware-`db` event was rejected as dishonest and
  useless, ADR-0015 Alternatives); **ADR-0016** re-adds the `EV_EFI_BOOT_SERVICES_APPLICATION`
  measurement that `LoadImageBuffer` (SourceBuffer load) skips, extending the chained image's
  Authenticode hash into the policy-configured PCR(s) — default `[4]`, explicit empty list
  disables — via `EFI_TCG2_PE_COFF_IMAGE`, so BitLocker's full default `0,2,4,11` profile
  auto-unlocks under the override (emulation-validated: reseal + 5/5 reboots to logon; fresh
  TPM → recovery). Both run through a new go-boot `EFI_TCG2_PROTOCOL` binding (fork tpba.8,
  additive `uefi/tcg2.go`) behind a 1-method `Extender` seam, with event construction in a
  pure-Go, host-tested `internal/tcgmeasure` (mirroring the Opal/`TCGTransport` split). **The
  authorization boundary is unchanged:** the measurement runs *after* `imageverify` accepts the
  image and *after* the `internal/tbstub` arm/verify gate — it is advisory-to-attestation and can
  never authorize anything, and it trusts no new external input (the measured CA / image bytes are
  the already-verified ones). The **fail-closed control on the measurement path is the new
  `require_tpm` policy flag** (default `false`): `true` → a missing `EFI_TCG2_PROTOCOL` or a failed
  `HashLogExtendEvent` refuses to boot (do not boot an image we cannot measure); `false` →
  best-effort (log loudly, boot the verified image *unattested* — never a silent fallback), so the
  override stays usable on TPM-less platforms while attestation-mandating deployments enforce it.
  Independent security review this session: **APPROVE** (security-neutral-to-positive, all findings
  LOW); the PCR-4 `PE_COFF` value is firmware-dependent and its real-hardware confirmation is an
  **open pre-condition, DEFERRED by decision this session** (ADR-0016 F-1). ADR-0015/0016 are
  **Proposed**, gated behind `-tags trustbroker`, and require the §5.3 human gate before Accepted;
  release builds stay override-free. See R-014.
...
| R-014 | `pba-override` is a Secure Boot override (authorizes an out-of-`db` image) | Gated — `-tags trustbroker` + explicit `pba-override` policy opt-in; fail-closed (verify→arm-one-buffer→restore). PCR-7 divergence characterized; scoped away from Windows/BitLocker by ADR-0012/0013. Accepted for the non-Windows/custom-loader path (ADR-0013, PR #88, 2026-07-05; review `evidence/security-review-records/2026-07-05-pba-override-trust-broker-82.md`). **ADR-0014 (Proposed)** extends it to the Windows path for **our-keys-only** platforms, gated on a mandatory provisioning obligation (BitLocker sealed with the PBA measured; PBA updates = reseal) + an OPEN pre-condition (verify the PCR-7 seal profile vs a primary Microsoft source + validate on real HW before production); new residual is operational (recovery-key prompt on PBA change), the fail-closed authorization properties unchanged. **ADR-0015/0016 (Proposed)** make the override measured/attestable — PCR-7 authority event (ADR-0015) + chained-image measurement into policy-configured PCR(s), default `[4]` (ADR-0016) — so BitLocker's full `0,2,4,11` profile auto-unlocks under the override; authorization is unchanged (measurement is post-verify/post-arm, advisory-to-attestation), the new `require_tpm` flag (default `false`) is the measurement-path fail-closed control, and the adopt/change residual is the same operational one-time reseal. Independent review APPROVE (findings LOW); real-HW PCR-4 confirmation DEFERRED (open pre-condition). All of ADR-0014/0015/0016 Proposed, pending the human gate; release builds stay override-free. #82. |
...
| 2026-08-29 | **TB2 exception + R-014 — `pba-override` becomes measured/attestable (ADR-0015 + ADR-0016, both Proposed; branch `fix/windows-handoff-demos`).** Two security-critical measured-boot changes extend the `-tags trustbroker` override (out of release builds). **ADR-0015:** before `StartImage` the override extends an `EV_EFI_VARIABLE_AUTHORITY` event into **PCR 7** naming a **PBA-owned** authority (variable `PbaOverride`, fixed PBA GUIDs + the embedded-trust-store `db` CA the signer chained to), closing the measured-boot gap where the override was invisible in PCR 7 / remote attestation (firmware records such an event for every `db`-validated image; the override recorded none). **ADR-0016:** the override re-adds the `EV_EFI_BOOT_SERVICES_APPLICATION` measurement that a SourceBuffer `LoadImageBuffer` skips — the chained image's Authenticode hash into policy-configured PCR(s) (`measure_image_pcrs`, default `[4]`, explicit empty disables, range-checked 0–23, `pba-override` entries only) via `EFI_TCG2_PE_COFF_IMAGE` — so BitLocker's default **PCR 0,2,4,11** profile reproduces and auto-unlocks under the override (emulation-validated: reseal + 5/5 reboots to logon; fresh TPM → recovery). Both go through a new go-boot `EFI_TCG2_PROTOCOL` binding (**fork tpba.7 → tpba.8**, additive new file `uefi/tcg2.go` = `GetTCG2` + `HashLogExtendEvent`, no change to existing files/asm — §6.6/R-006 additive-only) behind a 1-method `Extender` seam, event bytes built in a new pure-Go host-tested `internal/tcgmeasure`. **§4/entry points:** no new attacker-controlled input — the measured CA comes from the compiled-in trust store and the measured image bytes are the already-`imageverify`-accepted ones; one added firmware call (`HashLogExtendEvent`) on the already-privileged override path. **Authorization boundary UNCHANGED** — the measurement runs post-`imageverify`, post-`tbstub` arm/verify gate; it is advisory-to-attestation and can never authorize. New policy flag **`require_tpm`** (default `false`) is the measurement-path fail-closed control: `true` → missing TPM / failed extend refuses to boot; `false` → best-effort, log loudly + boot unattested, never a silent fallback. **No new threat, no rating change** — R-014 residual stays **Low** (authorization posture unchanged and still gated + fail-closed); the adopt/change-the-measurements residual (and any PBA change) is the same **operational one-time BitLocker reseal**, not a Secure Boot bypass. **Independent security review this session: APPROVE** (security-neutral-to-positive, all findings LOW): F-1 PCR-4 `PE_COFF` value is firmware-dependent, unproven off-hardware (ADR-0016 open pre-condition — real-HW validation **DEFERRED** by decision this session); F-2 cosmetic `ImageLocationInMemory` (log-only); F-3 default PCR-4 measure now also on the ADR-0013 non-Windows override path (human-gate confirm); F-4 = this compliance update. **Status: both Proposed / NOT Accepted** — §5.3 Security/Release Owner human gate is a merge pre-condition (as for ADR-0013/0014); release builds stay override-free. CRA matrix: ER-2/ER-1 attestation note added (shipped posture unchanged). Evidence: `evidence/security-review-records/2026-08-29-override-measured-boot-adr-0015-0016.md`. |
